...
import {loadInnerPackages, loadJson, updateJson, writeJson} from "./file-utils.js";
...
async function postClone(repoDirectory) {
    ...
    updateJson(path.resolve(process.cwd(), 'package.json'), 'dependencies', dependencies, 'rev-merge');

    const newTracking = [{repo: repoDirectory, path: repoDirectory}];
    updateJson(path.resolve(process.cwd(), 'inner-packages.json'), 'tracking', newTracking, 'append');
